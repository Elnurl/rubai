...
- **2FA Sign-in:** `app/(auth)/verify.tsx` handles Clerk `needs_second_factor` for all four strategies — TOTP authenticator, **email code** (`signIn.mfa.sendEmailCode` / `verifyEmailCode`), SMS, and backup code. The screen auto-sends the code on mount for email and SMS, exposes a tab switcher for any factors the account also supports, and surfaces user-friendly error copy. `sign-in.tsx` picks the preferred factor in order: TOTP → email → SMS → backup.
- **Google OAuth button:** Continue/Sign up with Google uses a custom multi-color `components/GoogleGIcon.tsx` (react-native-svg) so the brand mark renders correctly on Android (Ionicons' `logo-google` was monochrome and unclear).
