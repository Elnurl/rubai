...
- **Strict structured-output handling (Phase 1 of 3 — coach only):** every JSON-producing AI site already uses `response_format: json_schema` with `strict: true` and `additionalProperties: false`, but raw `JSON.parse(...) as Type` is a cast, not validation — failover via Anthropic tool_use can drop fields silently and OpenAI safety refusals (`message.refusal`) bypass content parsing entirely. `lib/strictJson.ts` provides `parseAndValidate(content, refusal, validator)` (refusal/empty/parse/validation taxonomy, throws typed `StrictJsonError`) and `strictJsonCompletion(req, params, validator, opts?)` which wraps `trackedCreate`, retries ONCE on parse/validation failure with a stricter system addendum citing the prior validation error, and never retries refusals. `coachResponseValidator` is a Zod mirror of `coachResponseSchema` (loose `passthrough` on nested objects so failover output isn't false-rejected, but `reply` is hard-required because `normalizeCoachOutput` cannot produce a usable turn without it). `POST /atlas/coach` uses `strictJsonCompletion`; refusals surface as 400, other failures as 500. `POST /atlas/coach/stream` uses `parseAndValidate` at end-of-stream (no mid-flight retry possible) and degrades gracefully on validation failure — the user already saw the streamed `reply` text via `ReplyTextExtractor`; the final SSE event simply lacks structured action fields. **Phase 2 (next session):** apply the helper to the remaining 9 endpoints (`/onboarding-chat` extractor, `/roadmap`, `/daily-plan`, `/adapt`, `/generate-title`, `/intake-questions`, `/intake-submit`, `/behavioral-profile`, `/evolve-roadmap`). **Phase 3 (later):** replace hand-written JSON schemas with `zodToJsonSchema(validator)` so the runtime validator and the OpenAI-side strict schema have a single source of truth.
